...
class ReviewPreprocessor:
    '''
    class for preprocessing our raw data set
    '''
    def __init__(self):
        self.raw_dir=DATA_PATHS['raw']
        self.processed_dir=DATA_PATHS['processed']
        os.mkdir(self.processed_dir,exist_ok=True)
    def clean_df(self,df):
        '''
        Apply cleaning:
         - remove Duplicates
         - remove missing reviews
         -normalize rating->change to numeric
         -normalize date->yyyy-mm-dd
        '''
        df=df.dropna(subset=["review"]) # Remove missing review
        df=df.drop_duplicates(subset=["review"])#Remove missing review
        if "rating" in df:
