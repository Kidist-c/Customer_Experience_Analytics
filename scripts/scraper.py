...
# imporinting Necessary libraries
...
class PlayStoreScraper:
    ''' Scraper class for Google Play Store review'''
    def __init__(self):# when intializing objects, objects gonna have config variables
        self.app_ids=APP_IDS
        self.bank_name=BANK_NAMES
    ...
    # function to get basic info or metadata about each app
    ...
    def scrape_reviews(self,app_id,count=400):
        '''
        scrape revies for specific app
        order it by newwest first
        tries to fetch up max_retries times if it fails
        '''
    ...
    def process_reviews(self, reviews_data, bank_code):
        '''
        process raw review data into dictionary for csv
        columns: review, rating, date, bank, source
        '''
        processed = []
        for review in reviews_data:
            # Get review text (empty string if missing)
            review_text = review.get('content', '')

            # Get rating (1-5, default 0)
            rating = review.get('score', 0)

            # Get review date, normalize to YYYY-MM-DD
